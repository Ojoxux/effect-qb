...
const main = async () => {
  ...
  try {
    ...
    await Bun.write(join(consumerDir, "index.ts"), [
      'import { Check, Column, ForeignKey, Function, Index, PrimaryKey, Query, Scalar, Table, Unique } from "effect-qb"',
      'import { Json as MysqlJson } from "effect-qb/mysql"',
      ...
      "  role: Column.text()",
      "}).pipe(",
      '  PrimaryKey.make("id").pipe(Pg.PrimaryKey.named("memberships_pkey")),',
      '  Unique.make(["userId", "role"] as const).pipe(Pg.Unique.named("memberships_user_role_key")),',
      '  Index.make("role").pipe(Pg.Index.named("memberships_role_idx"), Pg.Index.using("btree")),',
      '  ForeignKey.make("userId", () => users, "id").pipe(Pg.ForeignKey.onDelete("cascade")),',
      '  Check.make("memberships_role_check", Query.neq(Query.column("role", Query.type.text()), ""))',
      ")",
      "",
      ...
      "  }))",
      "})",
      "const lastPairValue = MysqlJson.get(",
      "  jsonDocs.payload,",
      "  MysqlJson.path(MysqlJson.key(\"profile\"), MysqlJson.key(\"pair\"), MysqlJson.index(-1))",
      ")",
      "const updatedPair = MysqlJson.set(",
      "  jsonDocs.payload,",
      "  MysqlJson.path(MysqlJson.key(\"profile\"), MysqlJson.key(\"pair\"), MysqlJson.index(-1)),",
      "  true",
      ")",
