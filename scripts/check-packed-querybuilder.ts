...
const main = async () => {
  ...
  try {
    ...
    await Bun.write(join(consumerDir, "index.ts"), [
      ...
      "",
      'const standardUsers = Std.Table.make("standard_users", {',
      "  id: Std.Column.uuid().pipe(Std.Column.primaryKey),",
      "  email: Std.Column.text()",
      "})",
      "",
      "const standardPlan = Std.Query.select({",
      "  id: standardUsers.id,",
      "  email: Std.Function.lower(standardUsers.email)",
      "}).pipe(Std.Query.from(standardUsers))",
      "",
      'const standardAliasUsers = Standard.Table.make("standard_alias_users", {',
      "  id: Standard.Column.uuid().pipe(Standard.Column.primaryKey)",
      "})",
      "",
      "const standardAliasPlan = Standard.Query.select({",
      "  id: standardAliasUsers.id",
      "}).pipe(Standard.Query.from(standardAliasUsers))",
      "",
