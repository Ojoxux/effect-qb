...
const main = async () => {
  ...
  try {
    ...
    await Bun.write(join(consumerDir, "index.ts"), [
      ...
      "  role: Column.text()",
      "}).pipe(",
      '  PrimaryKey.make("id").pipe(PrimaryKey.named("memberships_pkey")),',
      '  Unique.make(["userId", "role"] as const).pipe(Unique.named("memberships_user_role_key")),',
      '  Index.make("role").pipe(Index.named("memberships_role_idx"), Pg.Index.using("btree")),',
      '  ForeignKey.make("userId", () => users, "id").pipe(ForeignKey.onDelete("cascade")),',
